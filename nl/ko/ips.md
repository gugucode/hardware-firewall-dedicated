...
자주 묻는 질문(FAQ)은 **방화벽을 통해 허용되는 IP 범위는 어떻게 됩니까?**입니다. 다음 목록에는 하드웨어 방화벽(전용)에서 사용할 전체 IP 범위가 포함됩니다.
...
DAL01 및 서버 위치 둘 다에 대한 규칙을 추가하십시오. 서버가 AMS01에 있는 경우 DAL01 및 AMS01 둘 다의 트래픽을 허용하는 규칙을 추가해야 합니다.

Flex 이미지 프로비저닝(이미지 작성 및 서버 프로비저닝 모두)의 경우 DAL05도 허용해야 합니다.
...
|dal09|댈러스|텍사스|미국|10.2.112.0/20 and 10.3.192.0/24|
...
|wdc04|워싱턴 D.C.|-|미국|10.3.160.0/20 and 10.201.0.0/20|
