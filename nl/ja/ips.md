...
よくあるご質問の 1 つは、**どの IP 範囲を、ファイアウォール通過の許可対象にしますか?** というものです。以下のリストに、ハードウェア・ファイアウォール (専用) で使用する IP の全範囲を示します。
...
すべての TCP/UDP ポート<br>
EVault のポート固有情報については、[ここをクリック](https://console.bluemix.net/docs/infrastructure/Backup/evault-port-information.html#evault-port-information)してください。

## サービス・ネットワーク (バックエンド/プライベート・ネットワーク上)
DAL01 とサーバー・ロケーションの両方について、ルールを追加してください。ご使用のサーバーが AMS01 にある場合、DAL01 からのトラフィックと AMS01 からのトラフィックを両方とも許可するルールを追加する必要があります。

Flex Image プロビジョンの場合は (イメージ作成とサーバープロビジョンの両方)、DAL05 からのトラフィックも同様に許可する必要があります。
