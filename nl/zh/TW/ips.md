...
常見問題為**哪些 IP 範圍容許通過防火牆？**。下列清單包含使用硬體防火牆（專用）的 IP 完整範圍。
...
ICMP – ping（支援疑難排解）<br>
所有 TCP/UDP 埠<br>
如需 EVault 特定埠的資訊，[請按一下這裡](https://console.bluemix.net/docs/infrastructure/Backup/evault-port-information.html#evault-port-information)。

## 服務網路（後端/專用網路）
請務必新增 DAL01 及您伺服器位置的規則。如果您的伺服器是在 AMS01 中，您將需要新增容許 DAL01 及 AMS01 資料流量的規則。

針對「自訂影像佈建」（影像建立及伺服器佈建兩者），還需要容許 DAL05 的資料流量。
